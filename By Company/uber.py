...
def isPossible(self, n: int, edges: List[List[int]]) -> bool:
    # graph dict to store in degree nodes for each node as a set
    ...
    # if no odd degree nodes, return true
    if len(odd_degree_nodes) == 0:
        return True
    
    # if exactly 2 odd degree nodes
    if len(odd_degree_nodes) == 2:
        n1, n2 = odd_degree_nodes
        # connect n1 and n2
        if n1 not in graph[n2]:
            return True
        # connect n1, n2 with another node
        for node in range(1, n+1):
            if node != n1 and node != n2 and node not in graph[n1] and node not in graph[n2]:
                return True
    
    # if extactly 4 odd degree nodes
    if len(odd_degree_nodes) == 4:
        ...
        for u1, v1, u2, v2 in possible_pairs:
            if v1 not in graph[u1] and v2 not in graph[u2]:
                return True
            if v2 not in graph[u1] and v1 not in graph[u2]:
                return True
    return False
